...
void samcra3path(START,END,adj,max,datadj,numm,A,aantalnodes,path,hopcount,minimum_k,k_used)
...

{
  ...
  double ref, newvalue, *intermediate, endvalue, length;
  ...
  while(countdown < 1)
	...
	intermediate[0]= getdatad(sum[0][i],j-1) > datadj[0][i][m] ? datadj[0][i][m] : getdatad(sum[0][i],j-1);
		  if((A[0]/intermediate[0]) > newvalue)
		    newvalue=A[0]/intermediate[0];

	      length=1/intermediate[0]; /*or newvalue*/
		...
		//l=1;
		//while(l<=numm){
		  if((getdatad(sum[0][a],p-1))>endvalue)
		    mark=numm+1;
		  //l++;
		//}
		if(mark==(numm+1))
		...
		heap and increase the counter.
	      */
	      if((newvalue <=1.0)&&(length<=endvalue)&&(p==(counter[a]+1)))
		{
		  for(p=1;p<=counter[a];p++){
		    if(getdata(marked[a],p-1)==1){
		      if(length<=getlist(N[a],p-1)->key){
			insertdata(&previ[a],p-1,i);
			insertdata(&prevj[a],p-1,j);
			for(l=1; l<=numm; l++){
			  insertdatad(&sum[l][a],p-1,intermediate[l]);
			}
			insertdatad(&sum[0][a],p-1,length);
			decrease_key(&heap,getlist(N[a],p-1),length);
		  ...
		  if(p==counter[a]+1){
		    ...
		    insertdatad(&sum[0][a],p-1,length);
		    helppo=insert(&heap,&num,length,a,p);
		    counter[a]+=1;
		    insertlist(&N[a],counter[a]-1,helppo);
		  }
		  if((a==END)&&(length<endvalue))
		    endvalue = length;
		}/*newvalue*/
	    }/*for*/
	}/*else*/
