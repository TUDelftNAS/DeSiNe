...
int parse( network, n_ad, m_ad, nodes_ad, arcs_ad, cap_ad,
           source_ad, sink_ad, node_min_ad )

/* all parameters but the first are output */
char    **network;             /* network topology (replaces input file) */
...
int     k,                      /* temporary */
        ...
        cur_line;               /* number of current input line */
...
while ( ( in_line = network[cur_line] ) != NULL )
  {
  no_lines ++;
  cur_line ++;
